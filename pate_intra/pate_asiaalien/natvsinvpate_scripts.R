###HERE I MANUALLY CHECKED THE INVASIVE POINTS AND THEN BIND THEM WITH BIOCLIM VARIABLES

natvsinv<-read.csv(file="patenatvsinv.csv",header=TRUE,sep = ",")
coordinates(natvsinv)<-c("longitude","latitude")
natvsinvsp<-SpatialPoints(natvsinv)
natvsinvb<-raster::extract(a,natvsinvsp)
View(natvsinvb)
natvsinvb<-na.omit(natvsinvb)
natvsinvb<-raster::extract(a,natvsinvsp)
natvsinvc<-cbind(natvsinv,natvsinvb)
natvsinvc<-na.omit(natvsinvc)
write.csv(natvsinvc,"pate_asiavsext.csv")
patecalc<-read.csv(file="pate_asiavsext.csv",header=TRUE,sep = ",")
...
ylim[2] <- ylim[2] + 0.1
hist(y_E, proba = T,  xlim = xlim, ylim = ylim, col= 'white', density = 10, angle = 135, main = "H. patellifera (part)", xlab = "Between-Class PCA 1", ylab = "Density", cex.lab=1.5, cex.axis=1.5)
...
pca.env <- dudi.pca(patecalc[,c(8,9,13,14,15,16,18,21,22)], scannf = F, nf = 10) ##so can first make a pca of all, look at eigen values ect.
status <- as.factor(patecalc$status)
...
write.csv(niche_expansion_1, "comparison_diff_pate_natinv.csv")
pdf(file="pate_natinvdiffpate.pdf")
map_overlap(bet1$ls[,1], status)
dev.off()
View(eq.test)
View(niche.dyn.whole)
row <- cbind.data.frame(D.overlap, eq.test$p.D, niche.dyn.whole$dynamic.index.w[[1]],niche.dyn.whole$dynamic.index.w[[2]], niche.dyn.whole$dynamic.index.w[[3]])
names(row) <- c('D.overlap', 'eq.test', 'expansion', 'stability', 'Unfilling')
View(row)
...
pdf(file="niche_dyn_pate_part.pdf")
...
pdf(file = "pate_asia_pca_2ax.pdf")
...
dev.off()
pdf(file = "pate_asia_corr.pdf")
fviz_contrib(pca.env, choice="var", axes = 1:2, title="H. patellifera (part)")
dev.off()
