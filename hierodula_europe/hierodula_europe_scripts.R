###I MANUALLY MERGED THE TWO DATASETS AND THEN RUN THE ANALYSES, AS DONE WITH THE INTRA-SPECIFIC ONES

patevstrans<-read.csv(file="patevsbatt.csv",header=TRUE,sep = "")
library(ade4)
library(sf)
library(sp)
library(rgdal)
...
ylim[2] <- ylim[2] + 0.1
hist(y_E, proba = T,  xlim = xlim, ylim = ylim, col= 'white', density = 10, angle = 135, main = "patellifera vs transcaucasica", xlab = "Between-Class PCA 1", ylab = "Density", cex.lab=1.5, cex.axis=1.5)
...
status <- as.factor(patevsbatt$status)
pca.env <- dudi.pca(patevstrans[,c(8,9,13,14,15,16,18,21,22)], scannf = F, nf = 10) ##so can first make a pca of all, look at eigen values ect
...
scores.sp.ex <- data$CS1[which(data$status=='pate')]
scores.sp.nat <-data$CS1[which(data$status=='batt')]
#scores for the whole study region, same as presences as no considering whole study area
scores.clim.ex <- data$CS1[which(data$status=='pate')]
scores.clim.nat <-data$CS1[which(data$status=='batt')]
...
write.csv(row, "patevstranseu.csv")
pdf(file="greenpate_yellowtrans.pdf")
map_overlap(bet1$ls[,1], status)
dev.off()

pdf(file = "hiero_contr.pdf")
fviz_contrib(pca.env, choice="var", axes = 1:2, title="Hierodula in Europe")
dev.off()

pdf(file = "hiero_pca_2ax.pdf")
