...
limited_angui<-read.csv(file="limited_angui.csv",header=TRUE)
pca.env <- dudi.pca(limited_angui[,c(8,9,13,14,15,16,18,21,22)], scannf = F, nf = 10) ##so can first make a pca of all, look at eigen values ect.
status <- as.factor(limited_angui$status)
...
write.csv(row,"limited_niche_angui.csv")
pdf(file="angui_overlap_limited.pdf")
map_overlap(bet1$ls[,1], status)
dev.off()

###NICHE DYNAMICS PLOT

pdf(file="niche_dyn_angui_limited.pdf")
...
pdf(file = "angui_limited_pca_2ax.pdf")
...
dev.off()
pdf(file = "angui_limited_corr.pdf")
fviz_contrib(pca.env, choice="var", axes = 1:2, title="T. angustipennis (Anderson 2019)")
dev.off()

###MANTEL
pdf(file="mantel_lim_angui.pdf")
ecospat.mantel.correlogram(dfvar=limited_angui,colxy=2:3, n=100, colvar=c(8,9,13,14,15,16,18,21,22), max=50000, nclass=10, nperm=1000)
dev.off()
...
angui_n<-limited_angui[limited_angui$status == "N", ]
angui_e<-limited_angui[limited_angui$status == "E", ]

pdf(file="angui_limited_map.pdf")
ggplot() +
geom_map(
