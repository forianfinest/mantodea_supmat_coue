...
sinesis_angui<-read.csv(file="values_sinensis_angui.csv",header=TRUE)
...
####HERE, YOU CAN NOTICE A SUBLE CHANGE (PUTTING E AS "sinensis" AND N as "angustipennis")
...
###OTHER SUBTLE CHANGE HERE (save just the D value and eq test)
row <- cbind.data.frame(D.overlap, eq.test$p.D)
names(row) <- c('D.overlap', 'eq.test')
write.csv(row, "overlap_sinensis_angustipennis.csv")
pdf(file="sinensis_angui_overlap.pdf")
map_overlap(bet1$ls[,1], status)
dev.off()

###PCA PLOTS

pdf(file = "anguivssinensis_all_contr.pdf")
fviz_contrib(pca.env, choice="var", axes = 1:2, title="Tenodera in US (all)")
dev.off()

pdf(file = "anguivssinensis_all_pca_2ax.pdf")
