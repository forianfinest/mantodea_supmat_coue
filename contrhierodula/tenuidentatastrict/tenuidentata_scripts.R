...
ylim[2] <- ylim[2] + 0.1
hist(y_E, proba = T,  xlim = xlim, ylim = ylim, col= 'white', density = 10, angle = 135, main = "H. tenuidentata", xlab = "Between-Class PCA 1", ylab = "Density", cex.lab=1.5, cex.axis=1.5)
...
tenui<-read.csv(file="tenui.csv",header=TRUE)

###MAKE THE ANALYSES (BATES ET AL. 2020)

pca.env <- dudi.pca(tenui[,c(8,9,13,14,15,16,18,21,22)], scannf = F, nf = 10) ##so can first make a pca of all, look at eigen values ect.
status <- as.factor(tenui$status)
...
write.csv(niche_expansion_1, "tenui_ne.csv")
pdf(file="tenui.pdf")
