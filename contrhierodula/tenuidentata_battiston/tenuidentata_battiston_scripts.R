...
ylim[2] <- ylim[2] + 0.1
hist(y_E, proba = T,  xlim = xlim, ylim = ylim, col= 'white', density = 10, angle = 135, main = "H. tenuidentata sensu Battiston et al. (2018)", xlab = "Between-Class PCA 1", ylab = "Density", cex.lab=1.5, cex.axis=1.5)
...
batt<-read.csv(file="batt.csv",header=TRUE)

###MAKE THE ANALYSES (BATES ET AL. 2020)

pca.env <- dudi.pca(batt[,4:22], scannf = F, nf = 10) ##so can first make a pca of all, look at eigen values ect.
status <- as.factor(batt$status)
...
write.csv(niche_expansion_1, "ne_batt.csv")
pdf(file="tenui.pdf")
...
dev.off()
pdf(file = "batt_corr.pdf")
fviz_contrib(pca.env, choice="var", axes = 1:2, title="H. tenuidentata sensu Battiston et al. (2018)")
dev.off()

###MANTEL
pdf(file="mantel_batt.pdf")
ecospat.mantel.correlogram(dfvar=batt,colxy=2:3, n=100, colvar=c(8,9,13,14,15,16,18,21,22), max=50000, nclass=10, nperm=1000)
dev.off()
